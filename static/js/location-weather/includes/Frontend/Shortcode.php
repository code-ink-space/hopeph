...
class Shortcode {
	...
	public static function splw_html_show( $shortcode_id, $splw_option, $splw_meta ) {
		...
		if ( ! $appid ) {
			$weather_output = sprintf( '<div id="splw-location-weather-%1$s" class="splw-main-wrapper"><div class="splw-weather-title">%2$s</div><div class="splw-lite-wrapper"><div class="splw-warning">%3$s</div> <div class="splw-weather-attribution"><a href = "https://openweathermap.org/" target="_blank">Weather from OpenWeatherMap </a></div></div></div>', esc_attr( $shortcode_id ), esc_html( get_the_title( $shortcode_id ) ), 'Please enter your OpenWeatherMap <a href="' . admin_url( 'edit.php?post_type=location_weather&page=lw-settings#tab=api-settings' ) . '" target="_blank">API key.</a>' );
		...
		$data = self::get_weather( $query, $weather_units, $lw_language, $appid );
	 ...
	 * @api
	 */
	public static function get_weather( $query, $units = 'imperial', $lang = 'en', $appid = '' ) {
		$answer    = self::get_raw_weather_data( $query, $units, $lang, $appid, 'xml' );
	 ...
	 * @api
	 */
	public static function get_raw_weather_data( $query, $units = 'imperial', $lang = 'en', $appid = '', $mode = 'xml' ) {
